...
/*   Updated: 2023/08/22 19:28:02 by kslik            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_player(mlx_image_t *mlx_img, int center_x, int center_y)
{
	...
	while (x >= y)
	{
		if (center_y + y < 0 || center_y + x < 0 || center_y - x < 0 || center_y
			- y < 0)
			return ;
...
void	draw_line(t_all *all, int x1, int y1, int x2, int y2, int color)
{
	...
	while (++i <= steps)
	{
		if (x < 0 || x >= all->map.window_wid || y < 0
			|| y >= all->map.window_heig)
			return ;
			...
			while (++u < TILE_SIZE * MINIMAP_SCALE_FACTOR)
			{
				...
				while (++z < TILE_SIZE * MINIMAP_SCALE_FACTOR)
				{
					if (all->map.pure_map[i][j] == ' ')
						...
						mlx_put_pixel(all->mlx_img, j * TILE_SIZE
								* MINIMAP_SCALE_FACTOR + z, i * TILE_SIZE
								* MINIMAP_SCALE_FACTOR + u, get_rgba(0, 0, 0,
									255));
					else
						mlx_put_pixel(all->mlx_img, j * TILE_SIZE
								* MINIMAP_SCALE_FACTOR + z, i * TILE_SIZE
								* MINIMAP_SCALE_FACTOR + u, get_rgba(255, 255,
									255, 255));
				}
			}
		}
