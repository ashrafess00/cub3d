...
/*   Updated: 2023/08/23 11:36:24 by kslik            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_update_all(t_all *all)
{
	...
	new_player_x += all->player.move_side * sin(all->player.rotation_angle)
		* move_step;
	new_player_y -= all->player.move_side * cos(all->player.rotation_angle)
		* move_step;
	new_player_x += all->player.move_side * cos(all->player.rotation_angle
			+ M_PI / 2) * all->player.move_speed;
	new_player_y += all->player.move_side * sin(all->player.rotation_angle
			+ M_PI / 2) * all->player.move_speed;
	if (new_player_x < 0 || new_player_x >= all->map.window_wid
		|| new_player_y < 0 || new_player_y >= all->map.window_heig)
		return ;
	...
	draw_line(all, all->player.x, all->player.y, all->player.x
			+ cos(all->player.rotation_angle) * 100, all->player.y
			+ sin(all->player.rotation_angle) * 100, get_rgba(170, 200, 250,
				255));
	draw_casts(all, rays);
	put_settings(all);
}
